...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Using model:", model, "for image analysis:", isImageAnalysis)
    ...
    if (isImageAnalysis) {
      ...
      let result
      if (imageData && imageMetadata && !imageMetadata.name.startsWith("Generated:")) {
        try {
          // Validate image data
          if (!imageData.includes("data:image/") || !imageData.includes("base64,")) {
          ...
          console.log("[v0] Gemini response preview:", responseText.substring(0, 200))
...
${imageMetadata ? `Image info: ${imageMetadata.name} (${imageMetadata.type}, ${imageMetadata.dimensions?.width}x${imageMetadata.dimensions?.height})` : ""}`
    ...
    try {
      ...
      parsedResponse = {
        ...
        clarifyingQuestions: ["Could you be more specific about the desired changes?"],
      }
    }

    // Add some realistic processing time estimates based on edit type
    if (parsedResponse.editInstructions) {
      const complexityScore = parsedResponse.editInstructions.reduce((score, instruction) => {
      ...
      parsedResponse.estimatedProcessingTime = Math.min(Math.max(complexityScore * 10, 15), 90)
    }

...
export class BatchProcessingService {
  ...
  private static progressCallbacks: Map<
    string,
  ...
  private static async processBatchJob(job: BatchJob): Promise<void> {
    ...
    for (let i = 0; i < job.images.length; i += maxConcurrent) {
      ...
      // Update job progress
      job.progress.completed = job.images.filter(
        (img) => img.status === "completed"
      ).length;
      job.progress.failed = job.images.filter(
        (img) => img.status === "failed"
      ).length;
      job.progress.skipped = job.images.filter(
        (img) => img.status === "skipped"
      ).length;
    }
  }
  ...
  private static async processImage(
    ...
    try {
      ...
      // Safety check
      const safetyResult = await SafetyService.validateCommand(
        job.command,
        imageData
      );
  ...
  private static async mockProcessImage(
    image: BatchImage,
    command: string
  ...
  static getJobStats(jobId: string) {
    ...
    const avgProcessingTime =
      job.images
        ...
        .reduce((sum, img) => sum + (img.processingTime || 0), 0) /
